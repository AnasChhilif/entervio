    ...
    async def _analyze_with_llm(self, resume_text: str, job_description: str) -> Dict[str, Any]:
        ...
        try:
            ...
            completion = llm_service.groq_client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are an expert recruiter that outputs JSON."},
            ...
            return {"keywords": [], "critique": ["Could not generate critique due to error."]}
    ...
    def check_ats_compliance(self, resume_text: str) -> Dict[str, Any]:
        ...
        for section, patterns in required_sections.items():
            ...
            if found:
                ...
                recommendations.append(f"Add a clear '{section}' section to help ATS parsers identify your qualifications.")

        # 2. Text Length Check
        word_count = len(resume_text.split())
        if word_count < 200:
            issues.append("Resume too short")
            recommendations.append("Your resume seems very short (< 200 words). Add more detail to your experience.")
        elif word_count > 2000:
            issues.append("Resume too long")
            recommendations.append("Your resume is quite long (> 2000 words). Consider shortening it to keep recruiters' attention.")
